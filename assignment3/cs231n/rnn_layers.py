...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        temp_x = x[:, i, :]
        next_h, temp_cache = rnn_step_forward(temp_x, prev_h, Wx, Wh, b)
        h[:, i, :] = next_h
        prev_h = next_h
...
def rnn_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dh_temp = dh[:, i, :]
        temp_cache = cache[i]
        dx[:, i, :], dh0, dWx_step, dWh_step, db_step = rnn_step_backward(dh_temp + dh0, temp_cache)
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
    pass
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        temp_x = x[:, i, :]
        next_h, next_c, temp_cache = lstm_step_forward(temp_x, next_h, next_c, Wx, Wh, b)
        h[:, i, :] = next_h
        cache.append(temp_cache) 
    pass
...
def lstm_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        temp_dh = dh[:, i, :]
        temp_cache = cache[i]
        dx[:, i, :], dh0, dc, dWx_step, dWh_step, db_step = lstm_step_backward(temp_dh + dh0, dc, temp_cache)
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
    pass
